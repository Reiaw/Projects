...
<body>
    ...
    <h2 class="mt-4 mb-4">Scan Product</h2>
        <div id="scanner-container" style="width: 100%; max-width: 640px; height: 480px;"></div>
        <input type="text" id="barcode-value" class="form-control mt-3" readonly>
        <button onclick="startScanner()" class="btn btn-secondary mt-3 ml-2">Restart Scanner</button>
...
function submitBarcode() {
    var barcode = document.getElementById('barcode-value').value;
    if (barcode.trim() === "") {
        alert("No barcode detected. Please try again.");
        startScanner(); // Restart scanner if no barcode is detected
        return;
    }
    ...
    .catch(error => {
        ...
        startScanner(); // Restart scanner if there's an error
    });
}
